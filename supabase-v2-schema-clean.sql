-- OneTrack v2 Database Schema - Clean Version
-- Stores only essential transactional data, calculations done on frontend
...
-- RETAILERS TABLE
-- =============================================
-- Retailers that grow as orders are placed (shared across users)
CREATE TABLE retailers (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    name TEXT NOT NULL UNIQUE, -- Retailer name: "Target", "Walmart", "Local Store", etc.
...
CREATE TABLE orders (
    ...
    buy_date DATE,
    buy_price_cents INTEGER, -- Price paid in cents (per item)
    ...
    buy_retailer_id UUID REFERENCES retailers(id), -- User-specific retailer
    buy_notes TEXT,
    
    -- Sale information (if sold)
    sell_date DATE,
    sell_price_cents INTEGER, -- Price received in cents (per item)
    ...
    sell_retailer_id UUID REFERENCES retailers(id), -- User-specific retailer
    ...
    -- Financial calculations (computed fields - kept for performance)
...
-- Retailers policies (shared across users)
...
COMMENT ON TABLE retailers IS 'Retailers that grow as orders are placed (shared across users)';
...
-- Collection summaries, inventory views, and sold item aggregations
-- are now calculated on the frontend using the raw orders and items data.
-- This provides real-time accuracy and reduces database complexity.
