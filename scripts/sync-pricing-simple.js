...
console.log('🔄 Simple Pricing Sync - Extracting Raw & Graded Data...');
console.log('======================================================');
...
async function storeDetailedPricing(pricingRecords) {
  if (pricingRecords.length === 0) return 0;
...
async function processCardsBatch(cardIds, batchSize = 25) {
  ...
  for (let i = 0; i < cardIds.length; i += batchSize) {
    ...
    try {
      ...
      if (allPricingRecords.length > 0) {
        const stored = await storeDetailedPricing(allPricingRecords);
        totalPricingRecords += stored;
      }
...
async function syncDetailedPricing() {
  try {
    console.log('🚀 Starting simple pricing sync...');
    ...
    // Get all card IDs from the database (in chunks to avoid limits)
    ...
    console.error('❌ Simple pricing sync failed:', error.message);
    process.exit(1);
  }
}
