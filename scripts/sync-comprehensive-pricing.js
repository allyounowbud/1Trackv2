...
/**
 * Comprehensive Pricing Sync Script
 * Extracts ALL pricing data (raw and graded) for all cards with multiple variants
 */
...
console.log('💰 Comprehensive Pricing Sync');
console.log('=============================');
...
async function storePricingData(pricingRecords) {
  if (pricingRecords.length === 0) {
    console.log('⚠️ No pricing data to store');
    return;
  }
  
  console.log('💾 Storing comprehensive pricing data...');
  ...
  try {
    ...
    // Store new pricing data in batches
    const batchSize = 500;
    let stored = 0;
...
async function main() {
  try {
    console.log('🚀 Starting comprehensive pricing sync...');
    ...
    // Store comprehensive pricing data
    await storePricingData(pricingRecords);
    
    console.log('\n🎉 Comprehensive pricing sync completed!');
    console.log(`📊 Final stats:`);
    console.log(`   - Total cards: ${cards.length}`);
    console.log(`   - Pricing records: ${pricingRecords.length}`);
    console.log(`   - Coverage: ${((pricingRecords.length / cards.length) * 100).toFixed(1)}%`);
    
  } catch (error) {
    console.error('❌ Comprehensive pricing sync failed:', error.message);
    process.exit(1);
  }
}
